...
def draw_calendar_template():
    ...
    for i in range(2):
        for j in range(7):
            ...
            text = DAYS_OF_WEEK_SHORT[j], font=("Arial", 9), fill="black")
...
def do_small_calendar_month(stday, dsinmo, x):    # draws small calendars of previous month and next month
    ...
    for i in range(dsinmo):    
        canvas.create_text(x+ctr%7*W2, 88+ctr//7*12, text=DAY_NUMBERS[i], font=("Arial", 9), fill="black")
        ctr+=1
...
def do_small_calendar_headings():        
    ...
    if MV.month == 0:   # if current month is January
        canvas.create_text(X6, Y2, text = MONTH_STRINGS[MV.month] + "  " + str(MV.year - 1), font=("Arial", 12),\
                           fill = "black", tags="smcal")  # put December in left small calendar and previous year
        canvas.create_text(X7, Y2, text = MONTH_STRINGS[MV.month + 2] + "  " + str(MV.year), font=("Arial", 12),\
                           fill = "black", tags="smcal")  # next month in right small calendar        
    elif MV.month == 11:  # if current month is December
        canvas.create_text(X7, Y2, text = MONTH_STRINGS[MV.month + 2] + "  " + str(MV.year + 1), font=("Arial", 12),\
                           fill = "black", tags="smcal")  # put January in right small calendar and next year
        canvas.create_text(X6, Y2, text = MONTH_STRINGS[MV.month] + "  " + str(MV.year), font=("Arial", 12),\
...
def draw_month_and_year():  
    ...
    canvas.create_text(X9, Y3, text = MV.mth_obj[MV.month].name, font=("Arial", 38, "bold"), fill = COLOURS[MV.month], tags="mthandyr")
    canvas.create_text(X9, Y4, text = str(MV.year), font = ("Arial", 38, "bold"), fill = COLOURS[MV.month], tags="mthandyr")
...
def draw_main_body():
    ...
    for i in range(MV.mth_obj[MV.month].dysinmth):
        ...
        canvas.create_text(day.x + 15, day.y + 16, text = DAY_NUMBERS[i], font = ("Arial", 16), fill = "black", tags="day_element")
        ...
        if value3 != "":
            ...
            for line in word_list:
                canvas.create_text(day.x+60, day.y+38 + (ctr2 * 12), text=line, font=("Arial", 9), tags="day_element")  # display it to screen
...
def create_month_input_box():
    ...
    MV.month_label = tk.Label(root, text="MONTH:", font=("Arial", 12, "bold"))
...
def create_year_input_box():
    ...
    MV.year_label = tk.Label(root, text="YEAR:", font=("Arial", 12, "bold"))
...
def on_left_click(event):
    ...
    if MV.index == None:
        ...
        if MV.index != None:    # if a day was clicked, create a text input box
            MV.text_input = tk.Text(root, height=3, width=16, font=("Arial", 9), wrap=tk.WORD)
...
def create_buttons_and_text_boxes():
    ...
    minus_button = tk.Button(root, text=" - ", font=("Arial", 16), command=go_back)
    ...
    plus_button = tk.Button(root, text=" + ", font=("Arial", 16), command=go_ahead)
    plus_button.place(x=PLUS_BUTTON[0], y=PLUS_BUTTON[1])
    MV.my_widgets.append(plus_button)
    help_button = tk.Button(root, text="Help", font=("Arial", 14), command=display_help)
    help_button.place(x=HELP_BUTTON[0], y=HELP_BUTTON[1])
    MV.my_widgets.append(help_button)
    print_button = tk.Button(root, text="Print", font=("Arial", 14), command=lambda: print_canvas())
